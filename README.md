...
That's the whole thing. The first run downloads the model, extracts the 21 bundled probes against it (a one-time cost, cached to disk), and drops you into the TUI. Hit `/steer "happy" - "sad" 0.3` to steer toward the happy pole of `happy.sad`, or `[` / `]` to nudge temperature, or `Ctrl+A` to A/B compare the steered output against the unsteered baseline.
...
with SaklasSession("google/gemma-3-4b-it") as s:
    name, profile = s.extract("happy", baseline="sad")  # canonical name: "happy.sad"
...
21 probes across 6 categories, each backed by 45 curated contrastive pairs (topically disjoint, not minimal-word-swap — see CLAUDE.md for the generation discipline). Most probes are **bipolar**: the name carries both poles (`happy.sad`, `masculine.feminine`), the positive pole activates on α>0 and the negative pole on α<0. Monopolar probes have no named opposite.
...
|  > happy.sad   +0.30    |          Chat              |  Affect                |
|    formal_cas  +0.10    |                            |    happy.sad  #### .42 |
|                         |                            |    angry.calm ##- -.15 |
|  CONFIG                 |                            |  Epistemic             |
...
with SaklasSession("google/gemma-3-4b-it", device="auto") as session:
    # Extract a bipolar steering vector — the canonical name is "happy.sad"
    name, profile = session.extract("happy", baseline="sad")
    session.steer(name, profile)             # register (no alpha yet)

    # Generate with steering (positive α = happy pole, negative α = sad pole)
...
```python
_, hs = session.extract("happy", baseline="sad")    # canonical "happy.sad"
_, fc = session.extract("formal", baseline="casual")  # canonical "formal.casual"
session.steer("happy.sad", hs)
session.steer("formal.casual", fc)

session.generate("Hello.", alphas={"happy.sad": 0.2, "formal.casual": 0.1})  # both
session.generate("Hello.", alphas={"happy.sad": -0.2})                        # steer toward sad
...
# bipolar separator `.`.
name, profile = session.extract("happy")                         # monopolar
name, profile = session.extract("happy", baseline="sad")         # bipolar → "happy.sad"
name, profile = session.extract("high-context", baseline="low-context")  # → "high_context.low_context"
...
result.vectors           # {"happy.sad": 0.2} — snapshot of alphas used
...
DataSource.curated("happy.sad")                              # bundled
...
```python
collector = ResultCollector()
collector.add(result, concept="happy.sad", alpha=0.2, run_id=1)
...
Probe readings flatten to columns: `probe_honest.deceptive_mean`, `probe_honest.deceptive_std`, etc. Vector alphas flatten to `vector_happy.sad_alpha`.
...
saklas merge <name> <components> [-m] [-f] [-s]   # merge: saklas merge bard default/happy.sad:0.3,user/arch:0.4
```
...
cache_ops.install("a9lim/happy.sad@v1.2", as_=None, force=False, statements_only=False)
cache_ops.refresh(sel_parse("tag:affect"), model_scope="google/gemma-2-9b-it")
cache_ops.delete_tensors(sel_parse("happy.sad"), model_scope=None)
cache_ops.uninstall(sel_parse("happy.sad"), yes=False)
