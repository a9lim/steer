...
- `probes_bootstrap.py` — Walks `~/.saklas/vectors/default/` after `materialize_bundled()`. 28 bundled probes ship as per-concept folders in `saklas/data/vectors/`. `bootstrap_layer_means` caches at `~/.saklas/models/<id>/layer_means.safetensors`, stale when `neutral_statements.json` hash changes. MPS cache flushed between probe extractions.
...
28 curated concepts at **n=45 pairs each**, stored in `saklas/data/vectors/<concept>/statements.json`. Pairs are **topically disjoint** by design: Speaker A is saturated with concept-specific detail (names, terminology, sensory descriptions); Speaker B is a completely unrelated person/entity. This produces broader contrastive directions than minimal-word-swap pairs and is what `_REF_SCORE = 1/32` is calibrated against. 45 neutral statements in `saklas/data/neutral_statements.json` on the same generation discipline (affect-neutral, topically diverse, no stylistic voice). All generated via `scripts/regenerate_bundled_statements.py` on gemma-4-31b-it; regenerate with the same script if the generation prompt or `_N_PAIRS` changes.
