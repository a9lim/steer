...
- **Vectors scaled to mean hidden-state norm** at each extraction layer. Alpha directly represents the fraction of activation magnitude (e.g. alpha=0.5 means 50% perturbation at high-signal layers).
- **Monitor uses attention-weighted pooling**: after generation, a separate forward pass (`_encode_and_capture_all`) produces attention-weighted hidden states matching probe extraction. Mean-centered cosine similarities (both norms) remove baseline bias, bounded to [-1, 1]. The extra forward pass trades throughput for scoring consistency with probe extraction.
