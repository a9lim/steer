...
- `vector compare`: `--metric {euclidean,mahalanobis}` (default `euclidean`). Mahalanobis path requires `~/.saklas/models/<id>/{layer_means,neutral_activations}.safetensors` to exist on disk; `LayerWhitener.from_cache(model_id)` raises a `WhitenerError` with a populating-command hint when the cache is missing. `--ridge-scale FLOAT` (default 1.0) tunes the ridge multiplier on the regularized covariance.
...
- `--legacy` (v2.0 backcompat preset): on `tui`/`serve` flips `injection_mode="additive"` and `extraction_method="pca"` (passed through to `SaklasSession.from_pretrained`); on `vector extract` flips `--method pca`; on `vector compare` flips `--metric euclidean`. Mutually exclusive with the per-flag controls on the same verb (combination errors at parse time before model load). `_resolve_legacy_method(args)` (in `runners.py`) is the shared helper for the conflict check.
