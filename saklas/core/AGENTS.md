...
## triggers.py (v2.2 probe-gate addition)
...
`Profile.cosine_similarity(other, *, per_layer=False, whitener=None)` and `vectors.project_profile(..., whitener=None)` are the two integration points; both fall back to Euclidean when `whitener=None` so the low-level API is back-compat. `SaklasSession.whitener` is a lazy property; `bootstrap_probes` builds the whitener eagerly before extraction so v2.1+ default-extraction paths are end-to-end Mahalanobis. **Runtime projection default flipped to Mahalanobis in v2.2** — `_materialize_projections` consults `_resolve_projection_metric` and passes `self.whitener` to `project_profile` whenever the active metric is `"mahalanobis"`. `vector compare --metric` also flipped its default to `"mahalanobis"` in v2.2.

## generation.py — score_callback (v2.2)
