...
Returns a **profile**: `dict[int, Tensor]` covering every retained layer — no scores dict. `compute_layer_means` averages 90 neutral prompts for centering. `load_profile` dispatches on extension. `_template_overhead_cache` is keyed by `id(tokenizer)` — safe only while one tokenizer lives the session lifetime.
