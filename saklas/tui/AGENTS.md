...
- Commit (no-generation send): `/commit <text>` — the cross-terminal fallback for the `Ctrl+Enter` / `Alt+Enter` binding. Routes through the same `_commit_with_text` helper, so the role-aware dispatch (user-node → authored assistant turn; otherwise → new user turn) and the mid-gen `_pending_action` queueing are identical.
