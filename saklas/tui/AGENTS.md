...
- **`/transcript load <path> [--here|--merge] [--strict]`** routes through `Transcript.load` + `Transcript.import_into`. `--here` attaches at active node, `--merge` walks the deepest user-turn prefix, `--strict` raises on probe-hash drift. Guards (model/system-prompt mismatch, missing probes, content drift) print to chat with a `⚠` sigil via `warnings.catch_warnings`. Active node navigates to the imported leaf on success.
- **`/auto-regen [on|off|<mode>]`** stores both an enabled flag (`_loom_auto_regen_on`) and a mode (`_loom_auto_regen_mode`). Valid modes: `unsteered` / `inverted` / `reseed` / `cool` / `hot`, or `custom: <partial recipe>`. **`Ctrl+A`** (`action_ab_compare`) flips both `_ab_mode` and `_loom_auto_regen_on` together (decision 13: Ctrl+A is the on/off toggle, the visible side-by-side layout is the consequence). After every primary steered `done`, `_poll_generation` consults the mode: `unsteered` → existing `_start_shadow_generation` path (bit-identical to today's A/B); anything else → `_fire_auto_regen` which calls `session.regen_with_modifier(user_parent_id, mode, n=1)` in a worker, with a one-line "auto-regen → sibling …" system message.
