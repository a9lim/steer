...
Navigation is cursor-based: `↑↓`/`kj` move through every visible node, `←→`/`hl` fold-then-parent / unfold-then-first-child, `Tab`/`Shift+Tab` jump siblings, `g`/`G` first/active node, `Enter` activates + returns to chat, `Space` activates and stays, `Esc` exits compare/help/filter then backs to chat. Mutations: `e` edit, `b` branch, `r` regen, `d` delete, `s` star, `a` note, `f` fan. Analysis: `c` flips the detail pane to sibling-compare (`loom_helpers.format_compare`), `/` search with `n`/`N`, `p` edit prune filter inline, `?` toggle keymap overlay.
