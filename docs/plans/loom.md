...
- `SweepDrawer.svelte` and `saklas/tui/commands.py`'s `/sweep` are repointed to fan-out; the table view is deleted from the webui. Phase 5 work; ships in v2.4 as **its own PR** (touches both surfaces and is the only hard break in phase 5).
...
15. **Active node, not active leaf.** `active_node_id` may point at a leaf or an interior node. Interior selection happens when a user navigates back up the tree to send a new turn from there. Send semantics are role-aware: from an assistant node (leaf or interior), a new user turn attaches as a child; from a user node, send errors (suggest `/regen` or `/edit`).
